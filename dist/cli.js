 ...
 */
function ASSERT_LT(actual, expected, testName = '') {
    let threshold = 0.005;
 ...
 * @returns 1 if the assertion passes, 0 otherwise.
 */
function ASSERT_GT(actual, expected, testName = '') {
    let threshold = 0.01;
    if (actual > (expected - threshold)) {
        console.log(`\x1b[32m${testName}: Passed\x1b[0m`);
        return 1;
...
class License extends Metrics {
    ...
    async cloneRepository(cloneDir) {
        await git.clone({
            fs,
            http,
            dir: cloneDir,
    ...
    checkLicenseCompatibility(licenseText) {
    ...
    async extractLicenseInfo(cloneDir) {
        let licenseInfo = null;
        // Case-insensitive file search for README (e.g., README.md, README.MD)
        const readmeFiles = fs.readdirSync(cloneDir).filter(file => file.match(/^readme\.(md|txt)?$/i));
        ...
        const licenseFiles = fs.readdirSync(cloneDir).filter(file => file.match(/^licen[sc]e(\..*)?$/i));
 ...
 */
async function CorrectnessTest() {
    let testsPassed = 0;
    let testsFailed = 0;
    // Test 1
    ...
    const result2 = await correctness2.evaluate();
    ...
    const result3 = await correctness3.evaluate();
